...
//good candidate for pro dir
	 ...
	 * @since 2.4 #1035 Adds filter:aioseop_brackets on autogenerated descriptions (user & term).
	 *
	 * @param null $post
	 *
	...
	function get_main_description( $post = null ) {
		...
		if ( is_author() && $this->show_page_description() ) {
			$description = $this->internationalize(
				apply_filters( 'aioseop_brackets', get_the_author_meta( 'description' ) )
			);
			...
			if ( empty( $description ) ) {
				$description = apply_filters( 'aioseop_brackets', term_description() );
			}
			$description = $this->internationalize( $description );
		}
	 ...
	 * @since 2.4 #1035 Apply filter:aioseop_brackets.
	 *
	...
	function get_post_description( $post ) {
		...
		if ( ! $description ) {
			if ( empty( $aioseop_options['aiosp_skip_excerpt'] ) ) {
				$description = $this->trim_excerpt_without_filters_full_length( $this->internationalize(
					apply_filters( 'aioseop_brackets', $post->post_excerpt )
				) );
			}
			if ( ! $description
				&& isset( $aioseop_options['aiosp_generate_descriptions'] )
				&& $aioseop_options['aiosp_generate_descriptions']
			) {
				$content = $post->post_content;
				$content = empty( $aioseop_options['aiosp_run_shortcodes'] )
					? apply_filters( 'aioseop_brackets', $content )
	 ...
	 * @since 2.4 #1035 Adds filter:aioseop_brackets.
	 */
	function add_hooks() {
		...
		add_filter( 'aioseop_brackets', array( &$this, 'filter_brackets' ) );
	}
...
EOF;
	...
	/**
	 * Returns the passed value without brackets and its content.
	 * Basically this function removes shortcodes.
	 *
	 * @since 2.4
	 *
	 * @param string $text Value to modify.
	 *
	 * @return string
	 */
	public function filter_brackets( $text ) {
