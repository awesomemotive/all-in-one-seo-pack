 ...
 * @since 3.3.0 Complete refactoring.
 */
...
$(function () {
	...
	if ('undefined' !== typeof aioseopOGCharacterCounter) {
	...
	function aioseopAddEventListeners() {
		...
		if (('post.php' !== currentPage) && ('term.php' !== currentPage)) {
			return;
		}
	...
	function aioseopCountChars() {
		...
		for(var i = 0; i < countCharsFields.length; i++) {
			countCharsFields[i]();
	...
	function aioseopCheckIfAutogenerated() {
		if (
			(('post.php' !== currentPage) && ('term.php' !== currentPage)) || 
			(('on' !== aioseopCharacterCounterAutogenerateDescriptions) && ('aiosp_description' === inputField.attr('name')))  || 
			(('on' !== aioseopCharacterCounterAutogenerateDescriptions) && ('aioseop_opengraph_settings_desc' === inputField.attr('name')))  || 
		...
		switch (inputField.attr('name')) {
			...
			case 'aioseop_opengraph_settings_title':
				...
				break;
			default:
				var descriptionField = $('[name=aiosp_description]')[0].placeholder;
				cntField.val(+descriptionField.length);
				if(' ...' === descriptionField.slice(descriptionField.length-4)) {
					cntField.val(descriptionField.length-4);
				}
				break;
		}
	}
