 ...
 *
 * @since 3.0.0
 * @since 3.2.2 Add NextGen Album shortcode to `$conflicting_shortcodes'. #2774
 *
	...
	foreach ( $conflicting_shortcodes as $shortcode ) {
		...
		if ( has_shortcode( $content, $shortcode_tag ) ) {
			...
			if ( array_key_exists( $shortcode_tag, $shortcode_tags ) ) {
				$conflict_shortcodes[ $shortcode_tag ] = $shortcode_tags[ $shortcode_tag ];
			}
		}
	}

	if ( ! empty( $conflict_shortcodes ) ) {
		return aioseop_do_shortcode_helper( $content, $conflict_shortcodes );
	}
